...
/**
 * Servlet implementation class PostReply2
 */
@WebServlet("/PostReply2")
public class PostReply2 extends HttpServlet {
	...
	public void performTask(HttpServletRequest request, HttpServletResponse response) throws IOException{
		...
		try {
			...
			if(con != null) {
				...
				String sql = "insert into 2post_reply (topic_id,name,description,date_time)"+
				                     "values('"+id+"','"+name+"','"+desc+"','"+datetime+"')";
				String sql2 = "update 2post set num_reply=num_reply+1 where topic_id='"+id+"'";
				...
				if(return_val==1&&return_val2==1) {
					...
					out.print("<A HREF=\"2AllTopic.jsp\">ดูกระทู้ทั้งหมด</A><BR>");
					out.print("<A HREF=\"2ShowTopic.jsp?id="+id+"\">ย้อนกลับไปดูกระทู้ของคุณ</A>");
				} else {
					out.print("ไม่สามารถเก็บคำตอบลงกระทู้ได้ กรุณาลองใหม่!!!<BR>");
					out.print("<A HREF=\"2ShowTopic.jsp?id="+id+"\">กลับไปตอบกระทู้ใหม่</A>");
				}
				stmt.close();
				con.close();
			}
