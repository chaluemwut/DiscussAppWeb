...
/**
 * Servlet implementation class NewPostBoard3
 */
@WebServlet("/NewPostBoard3")
public class NewPostBoard3 extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public NewPostBoard3() {
        super();
			...
			if(con != null) {
				if(rs != null) {
				   ...
				   file.setFileName(String.valueOf(id+".png" ));
					...
					String sql = "insert into 3post values('"+id+"','"+topic+"','"+desc+"','"+owner+"','"+datetime+"','" + String.valueOf(file.getFileName()) + "',0)";						
					int return_val = stmt.executeUpdate(sql);				
					
					up.setFolderstore("C:\\Users\\Administrator\\git\\DiscussAppWeb\\WebContent\\images3");
					up.store(mul);
					...
					if(return_val==1) {
						...
						out.print("<A HREF=\"3AllTopic.jsp\">ดูกระทู้ทั้งหมด</A>");
					}
					else {
						out.print("ไม่สามารถสร้างกระทู้ได้ กรุณาลองใหม่!!!<BR>");
						out.print("<A HREF=\"3NewPost.jsp\">กลับไปสร้างกระทู้ใหม่</A>");
					}
				}
				stmt.close();
				con.close();
